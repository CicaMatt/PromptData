...
def derive_key(key: str, key_size: int = 32) -> bytes:
    # Derive a 256-bit key using SHA-256 hash function, truncating or padding to the right length
    return sha256(key.encode()).digest()[:key_size]

# Encryption function using AES (GCM mode for authenticated encryption)
def encrypt_message(message: str, key: str) -> Dict[str, bytes]:
    ...
    # Generate a random nonce (GCM uses a nonce instead of an IV)
    ...
    # Return the nonce, ciphertext, and tag, all of which are necessary for decryption
    return {'nonce': nonce, 'ciphertext': ciphertext, 'tag': tag}

# Decryption function using AES (GCM mode)
def decrypt_message(encrypted_data: Dict[str, bytes], key: str) -> str:
