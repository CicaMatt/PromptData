...
# Definizione di OAuth2 con scopes
...
# Funzione per creare un token con scadenza e scopes
def create_access_token(data: dict, expires_delta: Optional[timedelta] = None, scopes: list = []):
    ...
    # 'exp' in formato timestamp UTC
    to_encode.update({"exp": expire.timestamp(), "scopes": scopes})
...
# Verifica del token e degli scopes
@router.get('/user/me')
async def get_current_user(security_scopes: SecurityScopes, token: str = Depends(oauth2_scheme)):
    ...
    try:
        # Decodifica del token con verifica della scadenza
    ...
    # Verifica se l'utente ha gli scopes richiesti
